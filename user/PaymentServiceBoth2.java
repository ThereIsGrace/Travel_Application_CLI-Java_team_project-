...
public class PaymentServiceBoth2 {
	OrderDao od = new OrderDao();
	Scanner stdIn = new Scanner(System.in);
	FlightReservationDao frd =new FlightReservationDao();
	...
	public PaymentServiceBoth2(List<FlightBasketDto> abdList, LoginDto ld) {
		setLoginDto(ld);                   //로그인 객체를 복사
		for(FlightBasketDto fd : abdList) {
	...
	public void makeOrderFromBasket2() {
		for(int i = 0; i < fbdList.size(); i++) {
			OrderDao od = new OrderDao();
			od.makeFlightOrder(fbdList.get(i));
		}
	}
	...
	public void creditPay() {
		...
		if(number == secureNumber) {
			...
			if(fbdList != null) {
				frd.moveFromBasketToFlight(fbdList);  // 결제가 성공한 후에 예약 내역으로 옮김
			}
			od.deleteOrder(ld);
			...
			switch(num) {
			case 1: ReservationService rs = new ReservationService(ld); rs.seeReservation2(); break;
